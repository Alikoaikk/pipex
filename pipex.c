...
/*   Updated: 2025/07/05 14:46:27 by akoaik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	handle_main_errors(argc, argv, &var.infile, &var.outfile);
	...
	var.pid2 = fork();
	if (var.pid2 == 0)
		child2(argv[3], var.outfile, var.pipefd, envp);
	close(var.pipefd[0]);
	close(var.pipefd[1]);
	waitpid(var.pid1, NULL, 0);
	waitpid(var.pid2, &var.status, 0);
	return ((var.status & 0xff));
}
